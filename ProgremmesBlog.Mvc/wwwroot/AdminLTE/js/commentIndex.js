$(document).ready(function() {
                            ...
                            if (commentResult.Data) {
                                ...
                                $.each(commentResult.Data.Comments.$values,
                                    function(index, comment) {
                                        const newComment = getJsonNetObject(comment,
                                            commentResult.Data.Comments.$values);
                                        ...
                                        const newTableRow = dataTable.row.add([
                                            newComment.Id,
                                            newArticle.Title,
                                            newComment.Text.length > 75
                                            ? newComment.Text.substring(0, 75)
                                            : newComment.Text,
                                            `${newComment.IsActive ? "是" : "否"}`,
                                            `${newComment.IsDeleted ? "是" : "否"}`,
                    ...
                    success: function(data) {
                        ...
                        if (isValid) {
                            ...
                            dataTable.row(tableRow).data([
                                ...
                                commentUpdateAjaxModel.CommentDto.Comment.Text.length > 75
                                ? commentUpdateAjaxModel.CommentDto.Comment.Text.substring(0, 75)
                                : commentUpdateAjaxModel.CommentDto.Comment.Text,
                                `${commentUpdateAjaxModel.CommentDto.Comment.IsActive ? "是" : "否"}`,
                                `${commentUpdateAjaxModel.CommentDto.Comment.IsDeleted ? "是" : "否"}`,
                            ...
                            toastr.success(
                                `${commentUpdateAjaxModel.CommentDto.Comment.Id} no'lu yorum başarıyla güncellenmiştir`,
                                "Başarılı İşlem!");
                        } else {
                            let summaryText = "";
                        ...
                        success: function(data) {
                            ...
                            if (commentResult.Data) {
                                dataTable.row(tableRow).data([
                                    ...
                                    commentResult.Data.Comment.Text.length > 75
                                    ? commentResult.Data.Comment.Text.substring(0, 75)
                                    : commentResult.Data.Comment.Text,
                                    `${commentResult.Data.Comment.IsActive ? "是" : "否"}`,
                                    `${commentResult.Data.Comment.IsDeleted ? "是" : "否"}`,
    ...
    function getButtonsForDataTable(comment) {
        ...
        return `<button class="btn btn-info btn-sm btn-detail" data-id="${comment.Id
            }"><span class="fas fa-newspaper"></span></button>
                                <button class="btn btn-primary btn-sm mt-1 btn-update" data-id="${comment.Id
            }"><span class="fas fa-edit"></span></button>
                                <button class="btn btn-danger btn-sm mt-1 btn-delete" data-id="${comment.Id
