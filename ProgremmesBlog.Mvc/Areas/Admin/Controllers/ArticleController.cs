...
public class ArticleController : BaseController
{
	...
	public ArticleController(IArticleService articleService, ICategoryService categoryService, IMapper mapper,
		IImageHelper imageHelper, UserManager<User> userManager, IToastNotification toastNotification) : base(
	...
	public async Task<IActionResult> Add(ArticleAddViewModel articleAddViewModel)
	{
		if (ModelState.IsValid)
		{
			...
			var imageResult = await ImageHelper.Upload(articleAddViewModel.Title, articleAddViewModel.ThumbnailFile,
				PictureType.Post);
	...
	public async Task<IActionResult> Update(ArticleUpdateViewModel articleUpdateViewModel)
	{
		if (ModelState.IsValid)
		{
			...
			if (articleUpdateViewModel.ThumbnailFile != null)
			{
				...
				if (oldThumbNail != "postImages/defaultThumbnail.jpg") isNewThumbNailUploaded = true;
			}
			...
			if (result.ResultStatus == ResultStatus.Success)
			{
				if (isNewThumbNailUploaded) ImageHelper.Delete(oldThumbNail);
				_toastNotification.AddSuccessToastMessage(result.Message, new ToastrOptions
				{
					Title = "更新成功!"
				});
