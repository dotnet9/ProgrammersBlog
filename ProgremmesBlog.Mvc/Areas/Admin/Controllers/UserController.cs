...
public class UserController : BaseController
{
	...
	public async Task<IActionResult> Add(UserAddDto userAddDto)
	{
		if (ModelState.IsValid)
		{
			var uploadedImageDtoResult =
				await ImageHelper.Upload(userAddDto.UserName, userAddDto.PictureFile, PictureType.User);
			...
			if (result.Succeeded)
			{
				var userAddAjaxModel = JsonSerializer.Serialize(new UserAddAjaxViewModel
				{
					...
					{
						ResultStatus = ResultStatus.Success,
						Message = $"{user.UserName} 已成功添加用户名为name 的用户。",
						User = user
					},
	...
	public async Task<JsonResult> Delete(int userId)
	{
		...
		if (result.Succeeded)
		{
			...
			{
				ResultStatus = ResultStatus.Success,
				Message = $"{user.UserName} 已成功删除具有用户名的用户。",
				User = user
			});
		...
		foreach (var error in result.Errors) errorMessages = $"*{error.Description}\n";

		var deletedUserErrorModel = JsonSerializer.Serialize(new UserDto
		{
			ResultStatus = ResultStatus.Error,
			Message =
				$"{user.UserName} 删除具有用户名的用户时出现一些错误。\n{errorMessages}",
			User = user
		});
	...
	public async Task<IActionResult> Update(UserUpdateDto userUpdateDto)
	{
		if (ModelState.IsValid)
		{
			var isNewPictureUploaded = false;
			var oldUser = await UserManager.FindByIdAsync(userUpdateDto.Id.ToString());
			var oldUserPicture = oldUser.Picture;
			if (userUpdateDto.PictureFile != null)
			{
				var uploadedImageDtoResult =
					await ImageHelper.Upload(userUpdateDto.UserName, userUpdateDto.PictureFile, PictureType.User);
				userUpdateDto.Picture = uploadedImageDtoResult.ResultStatus == ResultStatus.Success
					? uploadedImageDtoResult.Data.FullName
					: "userImages/defaultUser.png";
				if (oldUserPicture != "userImages/defaultUser.png") isNewPictureUploaded = true;
			}
			...
			if (result.Succeeded)
			{
				if (isNewPictureUploaded) ImageHelper.Delete(oldUserPicture);
				...
				var userUpdateViewModel = JsonSerializer.Serialize(new UserUpdateAjaxViewModel
				{
					...
					{
						ResultStatus = ResultStatus.Success,
						Message = $"{updatedUser.UserName} 用户已成功更新。",
						User = updatedUser
					},
	...
	public async Task<ViewResult> ChangeDetails(UserUpdateDto userUpdateDto)
	{
		if (ModelState.IsValid)
		{
			...
			{
				if (isNewPictureUploaded) ImageHelper.Delete(oldUserPicture);
				_toastNotification.AddSuccessToastMessage("您的信息已成功更新。");
		...
		if (ModelState.IsValid)
		{
			...
			if (isVerified)
			{
				...
				if (result.Succeeded)
				{
					...
					_toastNotification.AddSuccessToastMessage("你的密码已成功更改。");
					return View();
				}
			...
			ModelState.AddModelError("", "请检查您输入的当前密码。");
			return View(userPasswordChangeDto);
		}
